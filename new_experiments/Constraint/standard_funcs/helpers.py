...
def project_A(matrix):
    return matrix.clip(-1, 1)

def project_mu(matrix):
...
def compute_fmeasure(Y_label, Y_pred):
    confusion_matrix = get_confusion_matrix(Y_label, Y_pred)
    return 1 - (2*confusion_matrix[1][1]/(confusion_matrix[1][1]+1-confusion_matrix[0][0]))
...
def compute_hmean(C):
    ...
    hm_part = 0.0
    for i in range(n):
        hm_part += (C[i, :].sum()/(C[i, i] + 1e-7))
    return 1 - (n/(hm_part+1e-7))
...
def compute_fmeasure_conf(confusion_matrix):
    return 1 - (2*confusion_matrix[1][1]/(confusion_matrix[1][1]+1-confusion_matrix[0][0]))
...
def compute_cov_value(C, target):
    return max(np.abs(np.array(compute_cov_each_class(C)) - target))

def compute_cov_each_class(C):
    n = C.shape[0]
    class_covs = []
    
    for i in range(n):
        class_cov = np.sum(C[:, i])
        class_covs.append(abs(class_cov))
    return class_covs

def compute_cov_class(C, epsilon, i, target, neg_cons=False):
    n = C.shape[0]
    if(not neg_cons):
        class_cov = np.sum(C[:, i]) - epsilon - target[i]
    else:
        class_cov = -np.sum(C[:, i]) - epsilon + target[i]
    return class_cov
